...
export default function AdminPage() {
  ...
  async function changeUnderdog(matchId: string, underdog: 'home' | 'away' | null) {
    await supabase.from('matches').update({ underdog }).eq('id', matchId)
    setMatches(prev => prev.map(m => m.id === matchId ? { ...m, underdog } : m))
  }
        ...
        <div>
          <div className="bg-white rounded-xl border border-gray-100 p-4 mb-4">
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div><label className="text-xs text-gray-500 block mb-1">Equipo A</label>
              ...
              <div><label className="text-xs text-gray-500 block mb-1">Equipo B</label>
            ...
            {underdogPreview && (
              <div className="bg-amber-50 rounded-lg px-3 py-2 text-xs text-amber-700 mb-3">
                {underdogPreview.homeRanking && underdogPreview.awayRanking ? (
                  <>
                    {newMatch.home} <strong>#{underdogPreview.homeRanking}</strong> FIFA · {newMatch.away} <strong>#{underdogPreview.awayRanking}</strong> FIFA
                    {underdogPreview.underdog
                      ? <> · ⭐ Favorito: <strong>{underdogPreview.homeRanking < underdogPreview.awayRanking ? newMatch.home : newMatch.away}</strong> · ⚡ No favorito: <strong>{underdogPreview.underdog}</strong></>
                      : <> · Sin datos suficientes</>}
                  </>
                ) : (
                  <span>⚠️ Uno o ambos equipos no están en el ranking FIFA — el no favorito quedará sin definir. Puedes igual agregar el partido.</span>
                )}
              </div>
            )}
            ...
            {matches.map(m => (
              <div key={m.id} className="bg-white rounded-xl border border-gray-100 px-4 py-3">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button onClick={() => removeMatch(m.id)} className="text-xs text-red-500 hover:text-red-700 ml-4 shrink-0">Eliminar</button>
                </div>
                {m.match_date && <div className="text-xs text-gray-400 mb-2">{new Date(m.match_date).toLocaleString('es-CL', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}</div>}
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-xs text-gray-500">No favorito:</span>
                  {[
                    { val: 'home' as const, label: m.home },
                    { val: 'away' as const, label: m.away },
                    { val: null, label: 'Sin bonus' },
                  ].map(opt => (
                    <button key={String(opt.val)} onClick={() => changeUnderdog(m.id, opt.val)}
                      className={`text-xs px-2 py-1 rounded border transition-all ${
                        m.underdog === opt.val
                          ? opt.val === null ? 'bg-gray-200 text-gray-700 border-gray-300' : 'bg-amber-500 text-white border-amber-500'
                          : 'bg-white text-gray-500 border-gray-200 hover:border-gray-400'
                      }`}>
                      {opt.val !== null ? '⚡ ' : ''}{opt.label}
                    </button>
                  ))}
                  {m.home_ranking && m.away_ranking && (
                    <span className="text-xs text-gray-400">({m.home} #{m.home_ranking} · {m.away} #{m.away_ranking} FIFA)</span>
                  )}
                </div>
              </div>
            ))}
